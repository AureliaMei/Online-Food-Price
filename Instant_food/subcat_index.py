...
# --- CONFIGURATION (Cấu hình cho Instant_food) ---
csv_folder = '/Users/my/Online Food Price/Instant_food/CSV'
lookup_file = '/Users/my/Online Food Price/Instant_food/cat_lookup_table.csv'
output_index_file = '/Users/my/Online Food Price/Instant_food/jevons_price_index.csv'
...
def main():
    ...
    print(f"Found {len(files)} files. Starting Jevons index calculation for Instant Food...")
    ...
    for file in files:
        ...
        try:
            ...
            # --- CÁC NGÀY TIẾP THEO ---
                ...
                # Duyệt qua từng nhóm hàng đã có trong lịch sử (Mì, Cháo, Phở...)
                ...
                # Kiểm tra nhóm mới xuất hiện (nếu có)
    ...
    if results:
        ...
        # Sắp xếp cột: Date đứng đầu, các nhóm Mì/Phở/Cháo xếp alphabe
        ...
        print(f"\n✅ Jevons Index calculated for Instant Food. Saved to: {output_index_file}")
